...
public class CameraActivity extends Activity{
	...
	//	SaveBitMap.isExistsFilePath();
		picture = new File(Environment.getExternalStorageDirectory()+"/soosokan",fname);
		setupViews();
	}
...
//			picture = new File(Environment.getExternalStorageDirectory(),fname);
		...
		protected void onPostExecute(String result) {
			Log.i(TAG, "zippicture===="+picture.getPath());
			 Bitmap bitmap =PictureUtils.getSmallBitmap(picture.getPath(), 400, 200);
		   PictureUtils.saveBitmap(bitmap, picture.getPath());
		}
	}
